...
def password_is_valid(request, password, confirm_password):
    ...
    if not password == confirm_password:
        messages.add_message(request, constants.ERROR, 'Suas senhas não se coincidem')
        return False
    if not re.search('[A-Z]', password):
        messages.add_message(request, constants.ERROR, 'Sua senha não contem letras maiúsculas')
        return False
    if not re.search('[a-z]', password):
        messages.add_message(request, 'Sua senha não possui letras minúsculas')
        return False
    if not re.search('[1-9]', password):
        messages.add_message(request, 'Sua senha não possui números')
        return False
    if not re.search('[!@#$%^&*]', password):
        messages.add_message(request, 'Sua senha não possui caracteres especiais')
        return False
    return True        
